...
- 修复：筛选面板最小轨道宽度由 480px 调整为 520px，三栏在内容容器 1040px 以下整体重排，
  为内部 166px / 300px 轨道、间距和面板内边距保留空间。
- 统一：IV 输入使用模块控件高度；768px 以下或粗指针设备恢复 44px 控件与候选菜单点击区域。
- 修复：结果错误提示允许换行，避免单行省略隐藏恢复操作所需的信息。
- 修复：自动完成输入保留右侧展开按钮的文本空间；筛选底部操作允许按实际宽度换行，
  IV 快捷按钮和筛选开关标签同步使用模块点击高度。
- 修复：由标题栏 ref 显式传入页签挂载位置，首次进入 Wild 就在标题栏显示 Generator/Searcher，
  不再等待一次交互后从正文跳到标题栏。
- 修复：等级范围独占筛选行，完整显示 `1..100`；英文长标签允许换行，数字输入使用 4px
  水平内边距，地点输入为展开按钮预留独立空间。
- 修复：地点初始显示实际选中项；输入草稿按游戏、遭遇类型、语言与有效索引区分，避免切换类型后
  显示旧地点。索引超出当前列表时，候选框与算法请求统一回到首项。
- 来源核对：上游 `Form/Gen3/Wild3.cpp` 的 Seed/Advance32Bit、地点自动完成设置与
  `Core/Gen3/Generators/WildGenerator3.hpp` 的 u32 参数；输入范围、Core 和 Worker 保持原契约。
- 已检查：外部 Chrome 的 `http://127.0.0.1:5173/` UI 预览；实际首次导航、Generator/Searcher
  样例、地点长文本搜索、Enter 选择、Escape 恢复、遭遇类型切换、等级范围错误及中英文/浅深主题切换。
  Generator 的 10 条样例首行顶边与表头底边相等，Searcher 返回 1 条样例；这些不是算法验收。
- 已检查：390、768、1280、1478、1920px 视口下 Wild 三块面板无横向溢出；390px 输入与菜单项
  高度约 44px，桌面约 30px；地点菜单与触发框等宽。1478px 下等级输入宽约 138px，英文筛选标签
  无文本裁剪。部分截图调用超时，相关尺寸只有 DOM 测量证据；本轮捕获的控制台 error 为空。
- 修复：Wild 桌面外壳按标题栏、工作区和页脚分配高度，结果表使用剩余空间并保留 180px 最小高度。
  控件过多或视口过短时自然撑开页面，不使用裁剪或多列独立滚动压缩控件；手机保持原有流式布局。
- 已检查：1920×1080 下整页高度由 1312px 收至 1080px；1478×679 与 1280×720 下结果表约 180px，
  结果区底边仍位于页脚之前。390px 下碎岩道具、Bike、同步队首与 Wild 4 设置无面板横向溢出。
  切回 ID 与定点时外壳为原 block 布局，ID 保持三栏同顶边，定点保持三块控制面板同排。
- 已通过：完整 `npm run verify`（179 个测试文件、625 项测试、Vite/PWA 生产构建）与
  69/69 项原生检查；加载本机 Emscripten 6.0.6 后的完整 `npm run build` 成功。
  `verify:full` 的首次 Wasm 阶段因终端未激活 emsdk 失败，后续构建已补齐此项。
- 待完成：缩放、粗指针真机、全部游戏档案与特殊地点尚未完整覆盖。
  修复已提交并推送为 `ebeb357`；尚未完成所有者验收。
