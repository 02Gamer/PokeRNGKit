...
- 静态 Seed 范围和帧数说明合并至输入面板，移除单独说明栏及不适用的“种族值”标题。
- Seed 工具在窄容器中同时重置输入区、结果区的网格行列，避免隐式列挤压及重叠。
- 手机四个主入口采用两列完整标签；输入与操作按钮恢复 44px 点击高度。
- 外部 Chrome `http://127.0.0.1:5173/` 已检查 390/1280/1920px 布局；1920px 下
  输入与结果并排，390px 下两者分别占满可用宽度并顺序排列。RS IDs 样例返回两行。
- 本次仅修改布局与静态说明位置，输入约束、Worker 与 RNG 算法保持现有实现；预览样例不作为算法验收。
