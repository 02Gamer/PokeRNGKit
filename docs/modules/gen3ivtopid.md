...
工程检查入口为 `npm test`、`npm run wasm:test:native` 和 `npm run wasm:build`。
0.1.0 收口已获得完整工程检查授权；算法验收仍必须等待 GitHub Actions 部署完成，
由项目所有者提供实际 URL 并明确授权后进行。
...
布局使用 `Gen3IvToPidPanel.css` 的 `ivtopid-page` 页面作用域：六项 IV 在桌面保持单行紧凑网格，性格、TID 和查找命令按实际内容宽度排列；窄视口再按单列或三列 IV 重排。输入、`haku-select`、查找/取消、结果操作和下拉选项在桌面为 `30px`，手机与粗指针恢复 `44px`，性格选项与触发框同宽并在框内截断。

结果标题将运行状态、错误或统计信息和操作放在同一行；结果表按 Seed、PID、生成方式、性别比例和 SID 的实际内容收窄，最末列不绘制右边框。模块专属的布局、密度和结果列规则只保留在该功能目录中；共享 Select 继续由公共样式负责。

0.1.0 收口允许错误、统计与长生成方式换行，避免隐藏诊断或截断方法名称。
工作区明确使用单列上下排列，覆盖宽屏共享双栏规则，避免 920px 工作区中的查找按钮被裁掉。
清空结果同步恢复就绪状态、零结果统计与空耗时，并提供明确的无障碍按钮名称。
外部 Chrome `http://127.0.0.1:5173/` 已确认手机输入和性格候选为约 44px，
预览样例三行结果可展示，清空后统计与表格一致。这些交互证据不作为 RNG 算法验收。
