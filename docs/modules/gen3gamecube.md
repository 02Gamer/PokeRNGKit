...
输入、下拉触发器、多选触发器、操作按钮和下拉选项在桌面为 `30px`，手机与粗指针恢复 `44px`。筛选器先按六项 IV 范围筛选，再按
“完美个体值 / 完美个体数”应用独立阈值条件；检索时仅枚举同时满足两层条件的互斥 IV 子范围。筛选底部把
“显示能力值”、Generator 的“取消筛选”和个体值计算器放在同一操作行，结果提示移到标题右侧，表格最后一列
不再显示右边界。

0.1.0 收口扩大桌面筛选面板最小宽度，底部开关和计算器占满筛选区并允许换行，
避免窄屏仍保留第二列位置。英文标签和错误提示允许换行；IV 输入及快捷按钮与主控件采用相同点击高度。
Generator/Searcher 切换时清空结果、统计和错误并恢复就绪，避免将生成器帧数重新显示为检索 Seed。
计算期间禁用模式切换，取消后可切换。输入范围、模板、Worker 和 RNG 算法保持现有实现。
结果保存本次计算的宝可梦种类，后续切换模板不会改变旧结果的特性名称和能力值换算。
外部 Chrome `http://127.0.0.1:5173/` 已确认 390px 底部操作区占满可用宽度、IV 输入约 44px，
Generator 预览样例返回八行；本地 UI 样例不作为算法验收。
